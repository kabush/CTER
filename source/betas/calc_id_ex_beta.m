...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.betas.fmri_id_ex_beta]);
    eval(['! rm -rf ',proj.path.betas.fmri_id_ex_beta]);
    disp(['Creating ',proj.path.betas.fmri_id_ex_beta]);
    eval(['! mkdir ',proj.path.betas.fmri_id_ex_beta]);
end
...
logger(['Calc. fMRI beta-series (EX Identify) of ',num2str(numel(subjs)),' subjects'],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);
...
for i=1:numel(subjs)
    ...
    %% ----------------------------------------
    %% Write scores to ex_betas for future MVPA
    id_ex_table = table(onset,...
                        trial_type,...
                        valence,...
                        arousal);
    filename = [proj.path.betas.fmri_id_ex_beta,'sub-',name,'_task-',task,'_ex_trials.tsv'];
    writetable(id_ex_table,filename,'FileType','text','Delimiter','\t');
    ...
    dlmwrite([proj.path.tmp,'out_path.txt'],proj.path.betas.fmri_id_ex_beta,'');
