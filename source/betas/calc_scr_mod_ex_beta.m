...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.betas.scr_mod_ex_beta]);
    eval(['! rm -rf ',proj.path.betas.scr_mod_ex_beta]);
    disp(['Creating ',proj.path.betas.scr_mod_ex_beta]);
    eval(['! mkdir ',proj.path.betas.scr_mod_ex_beta]);
end
...
for i=1:numel(subjs)
    ...
    save([proj.path.betas.scr_mod_ex_beta,'sub-',name,'_mod_ex_betas.mat'],'ex_betas');
