...
logger(['Prediction Affect of Modulate Scans  '],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);

%% Set-up Directory Structure for fMRI betas
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.mvpa.mod_exfb_affect]);
    eval(['! rm -rf ',proj.path.mvpa.mod_exfb_affect]);
    disp(['Creating ',proj.path.mvpa.mod_exfb_affect]);
    eval(['! mkdir ',proj.path.mvpa.mod_exfb_affect]);
end
...
for i = 1:numel(subjs)
    ...
    try
        ...
        %% Load beta-series
        base_nii = load_untouch_nii([proj.path.betas.fmri_mod_exfb_beta,'sub-',name,'_lss.nii']);
        ...
        %% Load labels
        filename = [proj.path.betas.fmri_mod_exfb_beta,'sub-',name,'_task-modulate_exfb_trials.tsv'];
