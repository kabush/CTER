...
logger([' Analyze MOD_FB Arousal Induction via SCR '],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);

%% Set-up Directory Structure
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.analysis.scr_mod_fb]);
    eval(['! rm -rf ',proj.path.analysis.scr_mod_fb]);
    disp(['Creating ',proj.path.analysis.scr_mod_fb]);
    eval(['! mkdir ',proj.path.analysis.scr_mod_fb]);
end
...
for i = 1:numel(subjs)
    ...
    for j = 1:Nscans
        ...
        filename = [proj.path.betas.scr_mod_fb_beta,'sub-',name,'_mod_fb_betas.mat'];
        load(filename);
        fb_betas = fb_betas.(['mod',num2str(j)])';
        betas = [betas;fb_betas];

        %% Load onset times for the task
        filename = [proj.path.betas.scr_mod_fb_beta,'sub-',name,'_mod_fb_onsets.mat'];
        load(filename);
...
logger(['Task time Predicting SCR (All Feedback trials)'],proj.path.logfile);
...
%% Save Model
save([proj.path.analysis.scr_mod_fb,'mdl.mat'],'mdl');
