...
logger([' Analyze MOD_EXFB Arousal Induction via SCR '],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);

%% Set-up Directory Structure
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.analysis.scr_mod_exfb]);
    eval(['! rm -rf ',proj.path.analysis.scr_mod_exfb]);
    disp(['Creating ',proj.path.analysis.scr_mod_exfb]);
    eval(['! mkdir ',proj.path.analysis.scr_mod_exfb]);
end
...
for i = 1:numel(subjs)
    ...
    for j = 1:Nscans
        ...
        filename = [proj.path.betas.scr_mod_exfb_beta,'sub-',name,'_mod_betas.mat'];
        load(filename);
        betas = betas.(['mod',num2str(j)])';
        all_betas = [all_betas;betas];

        %% Load onset times for the task
        filename = [proj.path.betas.scr_mod_exfb_beta,'sub-',name,'_mod_trial_types.mat'];
        load(filename);
        trial_types = trial_types.(['mod',num2str(j)]);
        all_trial_types = [all_trial_types;trial_types];

        %% Load affect information
        filename = [proj.path.betas.scr_mod_exfb_beta,'sub-',name,'_mod_valences.mat'];
        load(filename);
        vals = valences.(['mod',num2str(j)]);
        all_valences = [all_valences;vals];

        filename = [proj.path.betas.scr_mod_exfb_beta,'sub-',name,'_mod_arousals.mat'];
        load(filename);
...
%% Convert all values to double precision (lme requirement)
...
%% Construct (and name) columns of the data table
...
%% Fit the models (both GLM and GLMM)
...
%% Compare GLM vs GLMM for model fit controlling for complexity
...
%% Save Model
save([proj.path.analysis.scr_mod_exfb,'mdl.mat'],'mdl');
