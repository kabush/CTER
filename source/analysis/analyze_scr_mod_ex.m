...
logger([' Analyze MOD_EX Arousal Induction via SCR '],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);

%% Set-up Directory Structure for SCR betas
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.analysis.scr_mod_ex]);
    eval(['! rm -rf ',proj.path.analysis.scr_mod_ex]);
    disp(['Creating ',proj.path.analysis.scr_mod_ex]);
    eval(['! mkdir ',proj.path.analysis.scr_mod_ex]);
end
...
for i = 1:numel(subjs)
    ...
    for j = 1:Nscans
        ...
        filename = [proj.path.betas.scr_mod_ex_beta,'sub-',name,'_mod_ex_betas.mat'];
        load(filename);
...
%% Save Model
save([proj.path.analysis.scr_mod_ex,'mdl.mat'],'mdl');
